...
function render(){renderCalendar({day,plans,cloud,mode,tasks,miniMonth});}
async function load(){const version=++loadVersion;const requestedDay=day;try{const rows=cloud?await listCloud():localPlans();const taskRows=cloud?await listTasksCloud():localTasks();if(version!==loadVersion)return;tasks=taskRows;plans=rows.sort((a,b)=>a.start.localeCompare(b.start));render();sync(cloud?'✓ 共享看板 · 已同步 · 每 15 秒更新':'○ 本机保存 · 未开启跨设备同步');}catch(e){if(version===loadVersion){sync('共享未就绪 · 请执行 supabase-shared.sql 或检查网络');toast('读取失败：'+e.message);}}}
async function save(plan,previous){if(cloud)await writeCloud(plan,previous);else{const all=localPlans();const index=all.findIndex(p=>p.id===plan.id);if(index>=0)all[index]=plan;else all.push(plan);localStorage.setItem(localKey,JSON.stringify(all));}await load();}
...
const settings=()=>{const config=readConfig();$('cloud-url').value=config.url;$('cloud-key').value=config.key;$('settings-dialog').showModal();};
 ...
 $('import').onclick=()=>action(async()=>{const local=localPlans(),localTaskRows=localTasks();if(!local.length&&!localTaskRows.length){toast('本机没有需要导入的内容');return;}if(!confirm(`将 ${local.length} 条时间规划和 ${localTaskRows.length} 条任务公开到共享看板？已有同编号记录保留云端版本。`))return;const records=local.map(p=>({id:p.id,day:p.day,title:p.title,start:p.start,end:p.end,category:p.category,note:p.note,done:p.done}));if(records.length){const {error}=await cloud.from('shared_plans').upsert(records,{onConflict:'id',ignoreDuplicates:true});if(error)throw error;}if(localTaskRows.length){const {error}=await cloud.from('shared_tasks').upsert(localTaskRows.map(t=>({id:t.id,title:t.title,done:t.done})),{onConflict:'id',ignoreDuplicates:true});if(error)throw Error('任务导入失败，可重试；已导入的规划不会重复：'+error.message);}await load();toast('导入完成，本机原始记录仍保留');});
 $('export').onclick=()=>action(async()=>{const rows=cloud?await listCloud():localPlans();const taskRows=cloud?await listTasksCloud():localTasks();const url=URL.createObjectURL(new Blob([JSON.stringify({version:1,exportedAt:new Date().toISOString(),plans:rows,tasks:taskRows},null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download=`今日规划-${localDate()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);toast(`已导出 ${rows.length} 条计划`);});
window.addEventListener('focus',()=>{if(!busy)load();});window.addEventListener('storage',e=>{if([localKey,taskKey].includes(e.key)&&!cloud&&!busy)load();});window.addEventListener('online',()=>load());
setInterval(()=>{if(!document.hidden&&!busy){if(cloud)load();else render();}},15000);
...
async function listTasksCloud(){const {data,error}=await cloud.from('shared_tasks').select('*').order('created_at');if(error)throw Error('任务读取失败，请执行supabase-shared.sql：'+error.message);return data;}
async function saveTask(task,previous){if(cloud){const record={id:task.id,title:task.title,done:task.done};const query=previous?cloud.from('shared_tasks').update(record).eq('id',task.id).eq('updated_at',previous.updated_at):cloud.from('shared_tasks').insert(record);const {data,error}=await query.select();if(error)throw error;if(!data.length)throw Error('任务已在另一台设备改动，请刷新后重试');}else{const all=localTasks(),i=all.findIndex(t=>t.id===task.id);if(i<0)all.push(task);else all[i]=task;localStorage.setItem(taskKey,JSON.stringify(all));}await load();}
